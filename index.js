...
var num_crisis = 4; //Numero de crisis - Number of 'crisis'
var num_familia = 3; //Numero de familias por crisis - Number of 'familias' by 'crisis'
var max_afectados = 6; //Numero maximo de afectados por familia - Max number of 'afectados' by family
var prefix = 'generado_'; //Prefijo agredado en los archivos exportados - Prefix added in the export files
//***********************************************************
...
for (var j=0;j<num_crisis;j++){
	...
	var random_crisis = Math.floor(Math.random() * (name_type_location_crisis.length+1));
...
function generateFamily(parent_node,max_afectados){
	var rol_gender = {'Padre':'M','Madre':'F','Hijo':'M','Hija':'F'}; //Gender by role - Requered to set a corret name
	...
	for (var i=0; i<random_afectados;i++){
		...
		//Born city
		var random_city = Math.floor(Math.random() * (cities.length+1) );
